...
import com.ynthm.autoconfigure.excel.annotation.ExcelConstraint;
import com.ynthm.autoconfigure.excel.annotation.ExcelConstraint4Enum;
import com.ynthm.autoconfigure.excel.annotation.ExcelConstraintInterface;
...
public class ListSheetWriteHandler implements SheetWriteHandler {
  ...
  public String[] resolveExplicitConstraint(Field explicitConstraint, Map<String, Object> params) {
    if (explicitConstraint.isAnnotationPresent(ExcelConstraint4Enum.class)) {
      ...
      Class<? extends Enum<? extends Label>> enumFiled = annotation.enumFiled();

      List<String> result = new ArrayList<>();
      for (Enum<? extends Label> enumConstant : enumFiled.getEnumConstants()) {
        result.add(((Label) enumConstant).label());
      }
