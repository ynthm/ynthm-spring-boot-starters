...
function get_abs_dir() {
    ...
    # RDIR="$( dirname "$SOURCE" )" 如果run.sh在子目录添加  && cd ..
...
JAVA_OPTS="${JAVA_OPTS} -server -Xms256M -Xmx512M -Xss512K \
...
start()
{
    ...
    nohup "$JAVACMD" $JAVA_OPTS  -jar -Dio.netty.leakDetection.level=advanced ${APP_HOME}/$JAR $APP_ARGS $@> ${OUT_FILE} 2>&1 &
    echo $! > ${RUNNING_PID}
    popd > /dev/null
    ...
    if is_running; then
        ...
        echoRed "The ${APP_NAME} has not started - check log"
        exit 3
    fi
}
